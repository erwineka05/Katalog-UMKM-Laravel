                ...
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" id="title" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('title', $berita->title) }}" required>
                        </div>

                        {{-- Konten/Isi Berita --}}
                        <div class="mb-4">
                            <label for="content" class="block font-medium text-sm text-gray-700">Isi Berita</label>
                            <textarea name="content" id="content" rows="10" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('content', $berita->content) }}</textarea>
                        </div>

                        {{-- Upload Gambar --}}
                        <div class="mb-6">
                            <label class="block font-medium text-sm text-gray-700">Gambar Saat Ini</label>
                            <img src="{{ asset('storage/' . $berita->image_url) }}" alt="{{ $berita->title }}" class="h-32 object-cover rounded-md mt-2">

                            <label for="image" class="block font-medium text-sm text-gray-700 mt-4">Ganti Gambar (Opsional)</label>
